...
months.forEach(month => {
  monthlyTotals[month] = categoryData.reduce((sum, item) => sum + (item[month] || 0), 0);
});
  ...
  months.forEach(month => {
    pieChartData[month] = categoryData
      ...
      .map(item => ({
        name: item.category,
        value: item[month],
        percentage: ((item[month] / (monthlyTotals[month] || 1)) * 100).toFixed(1)
      }));
  });
  ...
  return (
    <div className="App">
        ...
        <ResponsiveContainer width="100%" height={400}>
          ...
          >
            ...
            {months.map((month, index) => (
              <Bar key={month} dataKey={month} fill={COLORS[index % COLORS.length]} name={`${month} 2025`} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="pie-container">
        {months.filter(month => monthlyTotals[month] > 0).map(month => (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                  nameKey="name"
                  label={({name, percentage}) => name && percentage ? `${name}: ${percentage}%` : ''}
                >
      ...
      <div className="insights-container">
        <h2>Key Insights</h2>
        <ul>
          <li>Total spending decreased slightly from February ({formatCurrency(monthlyTotals.February)}) to March ({formatCurrency(monthlyTotals.March)})</li>
          <li>February expenses were entirely in the Blitz category</li>
          <li>March spending was more diversified across 6 different categories</li>
          <li>Capability development became a major expense area in March (49.5% of total)</li>
          <li>Blitz category spending reduced significantly in March (from {formatCurrency(530.52)} to {formatCurrency(37.58)})</li>
        </ul>
      </div>
    </div>
  );
